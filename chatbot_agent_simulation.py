...
class Help_Center:
    ...
    def write_trial_results(self):
        mean_q_agent_time = self.df_individual_run['Q_Agent_Time'].mean()
        with open('trial_results.csv', 'a', newline='') as file:
            writer = csv.writer(file, delimiter=',')
            results_to_write = [mean_q_agent_time]
    ...
    def animation(self):
        ...
        # Animation for empirical queue estimate
        a_y0 = []
        ...
        Display.ax[0, 1].hist(np.array(self.df_individual_run['Q_Agent_Time']), bins=100)
